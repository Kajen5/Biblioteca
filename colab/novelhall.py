...
class Descargar():
    ...
    def __init__(self, base, titulo=None):
        ...
        if titulo is not None:
            ...
            titu = re.search(r'<h1>(.*?)</h1>', self.base, re.DOTALL)
    ...
    def obtener_link(self, start=0):
        ...
        lista = list()
        book = re.search(r'All Section Catalog</h3>(.*?)</div>',
                         self.base, re.DOTALL)

        # obtener capitulos
        enlaces = re.findall(r'<a href="(.*?)">(.*?)</a>',
                             book.group(1), re.DOTALL)
    ...
    def download_carpeta_book(self, cantidad=15):
    ...
    def otro(self, cola):
        for numero, titulo, links in self.lista_book:
            for link, titulo in links:
                chapter = re.search(r'/(\d+)\.html', link).group(1)
                if link in cola:
                    continue
                cola.append(link)
    ...
    def download_archivo(self, url, NOMBRE):
        ...
        data1 = self.leyendoPagina(
            "https://www.novelhall.com" + url, 160).decode('utf-8')
        try:
            titulo = re.search(
                r"<h1.*?>(.*?)</h1>", data1, re.DOTALL).group(1)
            content = re.search(r'htmlContent">\n(.*?)</div>',
                                data1, re.DOTALL).group(1)
    ...
    def guardar(self):
        ...
        for capitulo in self.lista_book:
            ...
            for link, titulo in chapter:
                chapt = re.search(r'/(\d+)\.html', link).group(1)
...
if __name__ == '__main__':
    enlace = "https://www.novelhall.com/Naked-Sword-Art-7597/"
