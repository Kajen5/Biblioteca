...
class Descargar():
    ...
    def obtener_link(self, start=0):
        ...
        lista = list()
        book = re.search(r'<div class="chapter-list">(.*?)</div>', pag_padre)

        # obtener capitulos
        enlaces = re.findall(r'<a href="(.*?)" title="(.*?)">', pag_padre)
        ...
        chapters = [lista[i:i+n] for i in range(0, len(lista), 100) ]
        for n, k in enumerate(chapter):
            self.lista_book.append([n, None, k])
    ...
    def hilo_busqueda_all_book(self, Padre, hilos, tupla, i):
        ...
        if i > 0:
            hilos[i - 1].join()
        print("Pagina padre cargada -->>" + str(book))
        for pagina in imagen_list:
            print(pagina)
        print("Link de imagenes cargada -->>" + str(book))
        print("\n")
    ...
    def download_archivo(self, url, NOMBRE):
        ...
        content = re.search(r'<div class="fr-view">\n(.*?)\n<a href="', data1, re.DOTALL).group(1)
...
if __name__ == '__main__':
    enlace = "https://wuxiaworld.online/so-pure-so-flirtatious"
